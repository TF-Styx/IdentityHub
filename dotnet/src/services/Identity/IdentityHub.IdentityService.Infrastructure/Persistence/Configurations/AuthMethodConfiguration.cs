    ...
    internal sealed class AuthMethodConfiguration : IEntityTypeConfiguration<AuthMethod>
    {
        public void Configure(EntityTypeBuilder<AuthMethod> builder)
        {
            ...
            builder.Property(am => am.Id)
                   .HasColumnName("Id")
                   .ValueGeneratedNever()
                   .HasConversion(outDB => outDB.Value, inDB => new AuthMethodId(inDB));
            ...
            builder.Property(am => am.AuthKey)
                   ...
                   .HasConversion(outDB => outDB.Value, inDB => new AuthKey(inDB));

            builder.Property(am => am.AuthData)
                   .HasColumnName("AuthData")
                   .IsRequired(false)
                   .HasConversion(outDB => outDB.HasValue ? outDB.Value.Value : null, inDB => string.IsNullOrWhiteSpace(inDB) ? null : new AuthData(inDB));
