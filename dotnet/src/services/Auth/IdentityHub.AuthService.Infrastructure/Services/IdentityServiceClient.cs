...
namespace IdentityHub.AuthService.Infrastructure.Services
{
    public sealed class IdentityServiceClient(HttpClient httpClient) : IIdentityServiceClient
    {
        ...
        public async Task<Maybe<UserResponse>> GetUserByLoginAsync(string login, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await _httpClient.GetFromJsonAsync<UserResponse>($"internal/api/users/by-login/{login}", cancellationToken);
